...
function setup() {
  getAccessSheet_();
  return 'Harbour Line private access system is ready.';
}
...
function verifyAccess(payload) {
  ...
  try {
    ...
    for (let i = rows.length - 1; i >= 1; i -= 1) {
      ...
      const url = requestId
        ? CONFIG.TYPEFORM_URL + '#request_id=' + encodeURIComponent(requestId)
        : CONFIG.TYPEFORM_URL;
...
function handleAdminDecision_(params) {
  ...
  try {
    ...
    sheet.getRange(rowNumber, COL.DECISION_NOTE).setValue(
      action === 'approve-send' ? 'Approved; code emailed automatically' : 'Approved; Ryan to send code'
    );
    ...
    if (action === 'approve-send') {
      if (!code) {
        return adminPage_('Approved', 'The request was approved, but the code could not be recovered. Ask the visitor to request access again.', false);
      }
    ...
    return adminPage_(
      'Request approved',
      'The code is active. Send it only to ' + email + '.',
      true,
      code || '[code unavailable; request again]'
    );
  } finally {
    lock.releaseLock();
  }
...
function sendOwnerNotification_(request) {
  ...
  const body = [
    ...
    'Approval links expire in ' + CONFIG.ADMIN_TOKEN_HOURS + ' hours and can be used once.',
    '',
  ...
  const htmlBody = `
    <div style="font-family:Arial,sans-serif;color:#1b2733;line-height:1.5;max-width:620px">
      ...
      <p style="color:#52616f">A visitor has requested private access.</p>
...
function sendVisitorCode_(email, name, code) {
  ...
  const portalUrl = ScriptApp.getService().getUrl() + '?tab=code';
  const body = [
    greeting,
    '',
    'Your Harbour Line access request has been approved.',
    '',
...
function isTemporarilyLocked_(email) {
  const count = Number(CacheService.getScriptCache().get(failureKey_(email)) || 0);
  return count >= CONFIG.FAILED_ATTEMPT_LIMIT;
}
...
function recordFailedAttempt_(email) {
  ...
  cache.put(key, String(count), CONFIG.FAILED_ATTEMPT_WINDOW_SECONDS);
}
...
function genericRequestResponse_() {
  return {
    ok: true,
    message: 'Your request has been recorded. Approved participants receive a private code by email.',
  };
}

function adminPage_(title, message, success, code) {
  const codeBlock = code
    ? `<div style="font-family:monospace;font-size:20px;letter-spacing:.04em;background:#f2f5f7;padding:14px;margin-top:20px">${escapeHtml_(code)}</div>`
    : '';
